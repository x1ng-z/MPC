...
if __name__ == '__main__':
    try:
        ...
        url =  'http://192.168.137.159:8080/AILab/python/modlebuild/3.do'  # sys.argv[1]
        modleId = 3#int(sys.argv[2])
        ...
        MPC = apc.apc(modle["P"], modle["p"], modle["M"], modle["m"], modle["N"], modle["APCOutCycle"], modle["fnum"],
                      ...
                      np.array(modle['funneltype']),modle['alphemethod'])
        isEnable = modle["enable"]
        resp_opc = requests.get("http://192.168.137.159:8080/AILab/python/opcread/%d.do" % modleId)
        ...
        while ((isEnable == 1) and (realvalidekey == modlevalidekey)):
            ...
            originalfunnels, decoratefunnels = tools.buildFunel(wp, deadZones, funelInitValues, MPC.N, MPC.p,
                                                                MPC.funneltype, MPC.PINF, MPC.NINF)
            comstraindmv, firstdmvs, originfristdmv = MPC.rolling_optimization(wp, y0, ypv, mv, limitmv, limitdmv,
                                                                               decoratefunnels)
            ...
            write_resp="" #= requests.post("http://192.168.137.15:8080/AILab/python/opcwrite.do", params=payload)
            if DEBUG:
            ...
            e, y_0N, dff = MPC.feedback_correction(np.array(modle_real_data['y0']), y0, mvfb,
                                                   np.array(modle_real_data["UFB"]),
                                                   ff,
                                                   (np.array(modle_real_data["FF"]) if ('FF' in modle_real_data) else []),
                                                   (np.array(modle_real_data["FFLmt"]) if (
                                                               'FFLmt' in modle_real_data) else []))
            y0 = y_0N
            ...
            print(payload)
            write_resp ="" #requests.post("http://192.168.137.159:8080/AILab/python/updateModleData.do", data=payload)
            if DEBUG:
                print(write_resp.text)
    except:
        traceback.print_exc()
