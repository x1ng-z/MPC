...
if __name__ == '__main__':
    try:
        ...
        url = sys.argv[1]#'http://localhost:8080/AILab/pythonsimulate/modlebuild/4.do'  # sys.argv[1]
        modleId = int(sys.argv[2])
        resp = requests.get(url)  # sys.argv[0]'http://localhost:8080/pythonsimulate/modleparam/1.do'
        ...
        while ((isEnable == 1) and (realvalidekey == modlevalidekey)):
            ...
            dmv_structe[dmv_structe==1]=originfristdmv

            #计算本次个dmv的增量

            tmp_dmv_ratio=pvMvEffect*dmv_structe
            UnCheckLimitDmv=np.sum(tmp_dmv_ratio,axis=0)/np.sum(matrixPvMvMapping,axis=0)
            #检测本次dmv的限制对不对
            ...
            writemv=origionstructmv+origionstruct_firstonedmv
