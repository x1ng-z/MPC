...
if __name__ == '__main__':
    ...
    url = 'http://localhost:8080/pythonsimulate/modlebuild/86.do'  # sys.argv[1]
    modleId = 86#int(sys.argv[2])
    resp = requests.get(url)  # sys.argv[0]'http://192.168.165.187:8080/python/modleparam/1.do'
    ...
    resp_opc = requests.get("http://localhost:8080/pythonsimulate/opcread/%d.do" % modleId)
    ...
    while ((isEnable == 1) and (realvalidekey == modlevalidekey)):
        ...
        # write_resp = requests.post("http://localhost:8080/pythonsimulate/opcwrite.do", params=payload)
        # if DEBUG:
        ...
        e, y_0N,dff = MPC.feedback_correction(np.array(modle_real_data['y0']), y0, mvfb, np.array(modle_real_data["UFB"]),
        ...
        write_resp = requests.post("http://localhost:8080/pythonsimulate/updateModleData.do", data=payload)
        if DEBUG:
            print(write_resp.text)
