...
if __name__ == '__main__':
    try:
        ...
        url = 'http://192.168.137.159:8080/AILab/pythonsimulate/modlebuild/3.do'  # sys.argv[1]
        modleId = 3#int(sys.argv[2])
        resp = requests.get(url)  # sys.argv[0]'http://192.168.137.159:8080/pythonsimulate/modleparam/1.do'
        ...
        resp_opc = requests.get("http://192.168.137.159:8080/AILab/pythonsimulate/opcread/%d.do" % modleId)
        ...
        while ((isEnable == 0) and (realvalidekey == modlevalidekey)):
            ...
            undecoratefunnels, decoratefunnels = tools.buildFunel(wp, deadZones, funelInitValues, MPC.N, MPC.p, MPC.funneltype, MPC.PINF, MPC.NINF)
            '''
            ...
            write_resp = requests.post("http://192.168.137.159:8080/AILab/pythonsimulate/opcwrite.do", params=payload)
            if DEBUG:
            ...
            write_resp = requests.post("http://192.168.137.159:8080/AILab/pythonsimulate/updateModleData.do", data=payload)
            if DEBUG:
                print(write_resp.text)
    except:
        traceback.print_exc()
