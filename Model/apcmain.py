...
if __name__ == '__main__':
    ...
    MPC = apc.apc(modle["P"], modle["p"], modle["M"], modle["m"], modle["N"], modle["APCOutCycle"], modle["fnum"],
                  ...
                  np.array(modle["R"]), np.array(modle['pvusemv']))
    ...
    while ((isEnable == 0) and (realvalidekey == modlevalidekey)):
        ...
        write_resp = requests.post("http://192.168.165.187:8080/AILab/python/opcwrite.do", params=payload)
        if DEBUG:
        ...
        limitmv = np.array(modle_real_data['limitU'])
        limitdmv = np.array(modle_real_data['limitDU'])
