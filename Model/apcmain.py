...
if __name__ == '__main__':
    ...
    url=sys.argv[1]#'http://192.168.165.187:8080/AILab/python/modlebuild/1.do'#sys.argv[0]
    ...
    tools=Help.Tools()
    MPC=apc.apc(modle["P"], modle["p"], modle["M"], modle["m"], modle["N"], modle["APCOutCycle"], modle["f"],np.array(modle["A"]),( np.array(modle["B"]) if("B" in modle) else []),np.array(modle["Q"]),np.array(modle["R"]))
    ...
    limitdmv=np.array([[1,1]])#TODO
    mv=np.array(modle_init_data['U'])
    mvfb=np.array(modle_init_data['UFB'])
    ff=np.array(modle_init_data['FF'])#前馈值
    ffdependregion=np.array(modle_init_data['FFLmt'])#前馈置信区间，不在这个区间内的ff,不可以用
