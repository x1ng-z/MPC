...
if __name__ == '__main__':
    ...
    url = 'http://localhost:8080/python/modlebuild/86.do'  # sys.argv[1]
    ...
    MPC = apc.apc(modle["P"], modle["p"], modle["M"], modle["m"], modle["N"], modle["APCOutCycle"], modle["fnum"],
                  ...
                  np.array(modle['funneltype']))
    isEnable = modle["enable"]
    resp_opc = requests.get("http://localhost:8080/python/opcread/%d.do" % modleId)
    ...
    while ((isEnable == 1) and (realvalidekey == modlevalidekey)):
        ...
        write_resp = requests.post("http://localhost:8080/python/opcwrite.do", params=payload)
        if DEBUG:
            print("opcwrite:"+write_resp.text)
        ...
        if DEBUG:
            print("updateModleData:"+write_resp.text)
