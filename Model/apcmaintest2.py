...
if __name__ == '__main__':
    try:
        ...
        url = 'http://localhost:80/python/modlebuild/86.do'  # sys.argv[1]
        ...
        resp_opc = requests.get("http://localhost:80/python/opcread/%d.do" % modleId)
        ...
        while ((isEnable == 0) and (realvalidekey == modlevalidekey)):
            ...
            write_resp = requests.post("http://localhost:80/python/opcwrite.do", params=payload)
            if DEBUG:
            ...
            write_resp = requests.post("http://localhost:80/python/updateModleData.do", data=payload)
            if DEBUG:
                print(write_resp.text)
    except:
        traceback.print_exc()
