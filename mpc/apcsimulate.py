        ...
        for pvi in range(p):
            for lineNum in range(2):
                ...
                for lineLimti in range(N):
                    ...
                    decoratefunnels[lineNum, pvi * N + lineLimti] = (Upki * lineLimti + Upbi) + funneltype[
                        pvi, lineNum] * funnelmaxminmatrix[lineNum]
...
class apc:
    ...
    def feedback_correction_for_simulate(self, Origionapc, matrixPvMvMapping, Y0Position, yreal, y0, lastmvfb,
                                         thistimemvfb, lastfffb, thistimefffb, ffdependregion, funnels):
        '''
                   function:
                        反馈矫正
...
class MinJ:
    ...
    def comput(self):
        ...
        linear_constraintu = LinearConstraint(self.B, (self.Umin - self.u0).transpose()[0, :],
                                              (self.Umax - self.u0).transpose()[0, :])
...
def main(input_data, context):
    ...
    if (input_data['msgtype'] == 'build'):
        if 'MPC' not in context:
            ...
            context['matrixPvMvMapping'] = np.array(input_data['pvmvmapping'])  # 未拆解之前的映射关系
            context['matrixPvFfMapping'] = (
                np.array(input_data["pvffmapping"]) if ("pvffmapping" in input_data) else [])  ##未拆解之前的映射关系
            context['pvMvEffect'] = np.array(input_data['pvmveffect'])  # pv对mv输出的影响占比矩阵

            context['origionA'] = np.array(input_data['origionA'])  # 原始未拆分响应矩阵(p,m,N)
            context['Origionapc'] = OrigionAPC(context['origionA'].shape[0], context['origionA'].shape[1],
                                               context['origionA'].shape[2])

            # 变下形状
            context['Origionapc'].setOrigion_A_step_response_sequence(
                context['tools'].build_A_respond(context['origionA']))
            context['origionB'] = (np.array(input_data['origionB']) if ("origionB" in input_data) else [])
            if context['origionB'] != []:
                context['Origionapc'].setOrigion_B_step_response_sequence(
                    context['tools'].build_B_respond(context['origionB']))

            context['Y0Position'] = context['tools'].getFirstY0Position(context['matrixPvMvMapping'])

            context['MPC'] = apc(input_data["P"], input_data["p"], input_data["M"], input_data["m"], input_data["N"],
                                 input_data["APCOutCycle"], input_data["fnum"],
                                 np.array(input_data["A"]), (np.array(input_data["B"]) if ("B" in input_data) else []),
                                 np.array(input_data["Q"]),
                                 np.array(input_data["R"]), np.array(input_data['pvusemv']),
                                 np.array(input_data['alphe']),
        ...
        if 'y0' not in context:
            ...
            context['limitmv'] = np.array(input_data['limitU'])  # 拆分映射的mv上下限
            context['origionstructlimitmv'] = np.array(input_data['origionstructlimitmv'])  # 原始结构的mv上下限
            context['limitdmv'] = np.array(input_data['limitDU'])  # ([[0.1,0.2],[0.1,0.2]])#拆分映射的dmv上下限
            context['origionstructlimitDmv'] = np.array(input_data['origionstructlimitDmv'])  # 原始结构的dmv上下限
            context['mv'] = np.array(input_data['U'])  # 原始结构的mv
            context['origionstructmv'] = np.array(input_data['origionstructmv'])  # 原始结构的mv
            context['mvfb'] = np.array(input_data['UFB'])  # 原始结构的fbmv
            context['originstructmvfb'] = np.array(input_data['originstructmvfb'])  # 原始结构的fbmv
            ...
            OUT = {
                ...
                , 'funelupAnddown': np.zeros((2, context['Origionapc'].p * context['Origionapc'].N)).tolist()
                , "writedmv": np.zeros_like(context['mv']).reshape(-1).tolist()
                , "writemv": context['mv'].reshape(-1).tolist()
                ,'msgtype': input_data['msgtype']
            }

            # realvalidekey = input_data['validekey']

            # isEnable = input_data['enable']
        elif 'y0' in context:
            undecoratefunnels, decoratefunnels = context['tools'].buildFunel(context['wp'], context['deadZones'],
                                                                             context['funelInitValues'],
                                                                             context['MPC'].N, context['MPC'].p,
                                                                             context['MPC'].funneltype,
                                                                             context['MPC'].PINF, context['MPC'].NINF)
            '''
            ...
            '''
            comstraindmv, firstdmvs, originfristdmv = context['MPC'].rolling_optimization(context['wp'], context['y0'],
                                                                                          context['ypv'], context['mv'],
                                                                                          context['limitmv'],
                                                                                          context['limitdmv'],
                                                                                          decoratefunnels)
            ...
            '''
            分配dmv到响应位置上去'''
            dmv_structe = context['matrixPvMvMapping'].copy()
            dmv_structe = dmv_structe.astype(np.float64)
            dmv_structe[dmv_structe == 1] = originfristdmv.reshape(-1)

            # 计算本次个dmv的增量

            tmp_dmv_ratio = context['pvMvEffect'] * dmv_structe
            UnCheckLimitDmv = np.sum(tmp_dmv_ratio, axis=0) / np.sum(context['pvMvEffect'].astype(np.float64), axis=0)
            UnCheckLimitDmv = UnCheckLimitDmv.reshape(-1, 1)
            # 检测本次dmv的限制对不对
            origionstruct_firstonedmv, origionstruct_origfristonedmv = context['MPC'].origionmvconstraint(
                context['origionstructmv'],
                UnCheckLimitDmv,
                context['origionstructlimitmv'],
                context['origionstructlimitDmv'])

            writemv = context['origionstructmv'] + origionstruct_firstonedmv.reshape(-1)
            # writemv =firstdmvs.reshape(-1) * selectmvmat.astype(int).reshape(-1)
            # payload = {'id': modleId, 'U': writemv.tolist(), 'validekey': modlevalidekey}
            # write_resp = requests.post("http://localhost:8080/AILab/pythonsimulate/opcwrite.do", params=payload)
            # if DEBUG:
            ...
            # resp_opc = requests.get("http://localhost:8080/AILab/pythonsimulate/opcread/%d.do" % modleId)
            ...
            e, y_0N, y_0send, _dff, funnel_send = context['MPC'].feedback_correction_for_simulate(context['Origionapc'],
                                                                                                  context['matrixPvMvMapping'],
                                                                                                  context['Y0Position'],
                                                                                       np.array(input_data[
                                                                                                    'origiony0']),
                                                                                       context['y0'],
                                                                                                  context['originstructmvfb'],
                                                                                       np.array(input_data[
                                                                                                    'originstructmvfb']),
                                                                                                  context['ff'],
                                                                                       (np.array(
            ...
            # limitmv = np.array(input_data['limitU'])#mv限制
            # limitdmv = np.array(input_data['limitDU'])#dmv限制
            # mv = np.array(input_data['U'])  # mv值
            # mvfb = np.array(input_data['UFB'])  # mv反馈
            context['limitmv'] = np.array(input_data['limitU'])  # 拆分映射的mv上下限
            context['origionstructlimitmv'] = np.array(input_data['origionstructlimitmv'])  # 原始结构的mv上下限
            context['limitdmv'] = np.array(input_data['limitDU'])  # ([[0.1,0.2],[0.1,0.2]])#拆分映射的dmv上下限
            context['origionstructlimitDmv'] = np.array(input_data['origionstructlimitDmv'])  # 原始结构的dmv上下限
            ...
            OUT = {
                    'dmv': originfristdmv.reshape(-1).tolist()
                        ...
                        ,'writemv':writemv.tolist()
                        ,'msgtype': input_data['msgtype']
                       }
            # write_resp = requests.post("http://localhost:8080/AILab/pythonsimulate/updateModleData.do", data=payload)
            # if DEBUG:
            #     print(write_resp.text)
